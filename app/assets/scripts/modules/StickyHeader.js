...
class StickyHeader {
  constructor() {
    ...
    if (this.siteHeader && this.pageSections.length > 0) {
      this.events();
    } else {
      console.error("StickyHeader: Required elements not found.");
    }
  }
  ...
  runOnScroll() {
    ...
    this.pageSections.forEach((el) => {
      if (el) {
        ...
        console.error("StickyHeader: Null element found in pageSections.");
      }
    });
  }
  ...
  calcSection(el) {
    if (
      ...
      if (
        ...
        let linkElement = document.querySelector(matchingLink);
        if (linkElement) {
          linkElement.classList.add("is-current-link");
        } else {
          console.error(
            `StickyHeader: Matching link ${matchingLink} not found.`
          );
        }
      }
    }
